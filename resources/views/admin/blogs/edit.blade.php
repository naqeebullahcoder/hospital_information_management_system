    ...
    <div class="card-body">
        <form action="{{ route("admin.blogs.update",[$blog]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }}">
                ...
                <input type="text" id="headline" name="headline" class="form-control" required value="{{ old('headline', isset($blog) ? $blog->headline : '') }}">
                @if($errors->has('title'))
            ...
            <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                ...
                <textarea type="text" rows="1" id="description" name="description" class="form-control" required >{{ old('description',$blog->description) }}</textarea>
            ...
            <div class="form-group {{ $errors->has('picture') ? 'has-error' : '' }}">
                ...
                <input type="file" rows="2" id="picture" name="picture[]" class="form-control" value="{{ old('picture', isset($blog) ? $blog->picture : '') }}" multiple>
                @if($errors->has('description'))
                    <em class="invalid-feedback">
            ...
            <div class="form-group {{ $errors->has('date') ? 'has-error' : '' }}">
                ...
                <input type="date" id="date" name="date" class="form-control" required value="{{ old('date', date('Y-m-d', strtotime($blog->date))) }}">
                @if($errors->has('date'))
                    <em class="invalid-feedback">
                        {{ $errors->first('date') }}
                    </em>
