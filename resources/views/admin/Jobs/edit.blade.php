...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route("admin.jobs.update",[$jobs]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="form-group {{ $errors->has('type') ? 'has-error' : '' }}">
                    ...
                    <select name="type" id="type" class="form-control select2">
                        @foreach($job_types as $job_type)
                            <option value="{{$job_type->id}}" {{ old('type', $jobs->type) == $job_type->id ? 'selected' : '' }}>
                ...
                <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                    ...
                    <select name="status" id="status" class="form-control select2">
                        @foreach($status as $active_status)
                            <option value="{{$active_status->id}}" {{ old('status', $jobs->status) == $active_status->id ? 'selected' : '' }}>
